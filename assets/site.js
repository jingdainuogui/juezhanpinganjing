const Site = (() => {
  const backgroundCandidates = ["bg.jpg", "bg.png", "bj.png"];
    ...
    article.innerHTML = `
      ...
      >
        <div class="gallery-thumb-wrap">
          <img src="${src}" alt="式神图鉴 ${index + 1}" loading="lazy" class="gallery-thumb" />
  ...
  function initGallery() {
    ...
    const images = window.SHIKIGAMI_IMAGES.map((name) => `ss/${name}`);
    const batchSize = 18;
    let cursor = 0;
    let loading = false;
    ...
    const renderBatch = () => {
      ...
      nextImages.forEach((src, idx) => {
        fragment.appendChild(createGalleryCard(src, cursor + idx));
      });
      ...
      if (cursor >= images.length && sentinel) {
        sentinel.textContent = "结界底部已抵达，所有式神均已显现。";
      }
    };
  ...
  function createPortfolioCard(item, index) {
    ...
    article.innerHTML = `
      ...
      >
        <div class="gallery-thumb-wrap">
          <img src="${item.cover}" alt="${item.title}" loading="lazy" class="gallery-thumb portfolio-thumb" />
          <div class="gallery-glow"></div>
        ...
        item.innerHTML = `
          ...
          <form data-reply-index="${index}" class="relative z-10 mt-4 flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
            <div class="flex items-center justify-between gap-3">
              ...
              <span class="text-xs text-slate-300/56">本地演示回复，保存在当前浏览器</span>
            </div>
